...
Key ratio: HBM is ~23x faster than PCIe for large transfers (217 GB/s vs 9.4 GB/s). V1's decision to eliminate GPU<->CPU KV swap in favor of recompute follows from this. The swap path was bottlenecked by PCIe, not HBM.
...
- High sm% + high mem% = both compute and memory controllers active (typical of transformer workloads)

Caveat on sm%: this metric reports the percentage of time any SM had at least one warp executing. Both fully saturated SMs and a single active warp per SM can show sm% ~ 100%. Necessary but not sufficient indicator of compute utilization.
...
Since 1 << 203 (ridge point), decode is memory-bandwidth-bound. The GPU achieves at most ~0.5% of peak compute during decode. Every output token re-reads the entire model weights from HBM.
...
Since 2048 >> 203 (ridge point), prefill is compute-bound. A single read of the model weights is amortized across all prompt tokens, turning the same memory read into 2048x more useful work.
...
T4 peaks at ~23 TFLOPS FP16 (35% of theoretical 65 TFLOPS). At small batch sizes the GPU is severely underutilized due to insufficient parallel work to occupy all SMs. The theoretical peak assumes perfectly saturated tensor cores.
...
The dominant GEMM is compute-bound and operating near Tensor Core saturation. Nsight recommendation: "shift work from compute to another unit."
...
Attention is not compute-saturated and not DRAM-bound. It is latency-limited and cache-heavy, relying on L1/TEX paths. High SM active cycles (~95%) with low compute throughput (~28%) indicates dependency chains, warp synchronization, and limited ILP.

### Decode Kernel Characterization

Kernel launch profile:
- 53,000 `cudaLaunchKernel` calls (scales with tokens x layers x ops/layer)
...
Decode is memory-bandwidth-bound at 96% DRAM throughput. The GPU spends almost all its time streaming weights from HBM. Nsight recommendation: "shift work from memory to another unit."
...
Confirms the roofline prediction from Section 2: prefill is compute-bound (AI >> ridge point), decode is memory-bound (AI << ridge point).
...
```
Per-block KV size = 36 KiB/token x 16 tokens/block = 576 KiB per block
...
Block size formula (from vllm/v1/kv_cache_interface.py):
  ...
  x 36 layers = 589,824 bytes (~576 KiB)
```

### KV Cache Pool at Various Utilizations (Qwen2.5-3B, T4 15GB)

Overhead varies by utilization level. vLLM allocates more memory for CUDA graphs and activation buffers at higher utilization settings. Values below are from vLLM's actual reported allocations, not from a fixed overhead subtraction.
...
| 0.95 | 14.59 GB | ~6.15 GB | ~10,600 | ~169,600 |

Note: vLLM's self-reported capacity at default utilization (0.90) is 217,312 tokens. This is higher than the table's ~148,800 because vLLM's internal profiler is more precise about available memory after model loading and CUDA context allocation.
...
No Day 3 fragmentation simulation artifact found.

### Why Paged Allocation Matters

Traditional KV cache allocation (pre-PagedAttention) pre-allocates contiguous memory for each sequence's maximum possible length:

- Internal fragmentation: a sequence generating 100 tokens but allocated for 2048 wastes 95% of its reservation.
- External fragmentation: released contiguous chunks create holes unusable by sequences needing different sizes.

PagedAttention eliminates both by allocating KV cache in fixed-size blocks (16 tokens each), mapped via a block table. Blocks can be non-contiguous in physical memory but appear contiguous to the attention kernel via indirection.

Measured impact (from Week 2 experiments): at 0.45 utilization with 1,034 blocks, the block pool was fully utilized across 32 concurrent sequences, each holding a different number of blocks depending on decode progress. Without paging, the same 32 sequences at max_tokens=1900 would require 32 x ceil(1900/16) = 32 x 119 = 3,808 blocks, exceeding the pool by 3.7x.

---

## Section 6: Interconnect & TP Cost Preview

No Day 4 interconnect/TP artifact found. The T4 is a single-GPU setup.
...
On T4 with PCIe at ~9 GB/s, TP across multiple GPUs would be severely bandwidth-limited compared to NVLink-equipped GPUs (A100: 600 GB/s, H100: 900 GB/s). T4 is a single-GPU inference card. TP across PCIe would negate throughput gains from additional compute.
...
Measured with Nsight Compute: decode GEMV hits 96% DRAM throughput, prefill GEMM hits 84% compute throughput. Matches the roofline prediction (AI=1 for decode vs AI=2048 for prefill at 2048-token prompt).

### 2. T4 peaks at ~35% of theoretical compute

23 TFLOPS achieved vs 65 TFLOPS theoretical. Power-limited at 70W TDP. Plan for this ceiling, not the spec sheet.
...
At batch=1, the GPU achieves 0% of peak. At batch=1024, 35%. Continuous batching keeps the batch size large enough to amortize memory access across multiple requests.

### 5. KV cache is the binding constraint for concurrent serving

At 36 KiB/token for Qwen2.5-3B, a 2048-token sequence consumes ~72 MB of KV cache. With ~1-5 GB available for KV (depending on utilization setting), the system serves 11-80 concurrent sequences before exhausting the block pool.

### 6. Attention is neither compute-bound nor memory-bound

Nsight Compute shows FMHA at 28% compute throughput and 6.5% DRAM throughput, but 83% L1/TEX throughput. Latency-limited with dependency chains, not throughput-limited. This is why Flash Attention's algorithmic improvements (tiling, reduced HBM reads) matter more than raw bandwidth.

### 7. sm% is misleading without ncu

nvidia-smi dmon showed sm=89-100% during workloads, but Nsight Compute revealed the decode kernel is actually memory-stalled 47% of the time. sm% only measures "was any warp active." It does not distinguish between fully saturated SMs and a single active warp per SM.
