...
AWQ identifies the ~1% of weights that matter most for minimizing layer output error (activation-aware). These weights are scaled up before quantization, then scaled back at runtime, preserving signal for critical weights. Naive quantization treats all weights equally and loses more signal at the same bit width.

GPTQ quantizes all weights using second-order activation-aware error minimization, layer by layer. No preferential scaling. More compute-intensive calibration than AWQ, but designed for INT4 where you need every bit of accuracy recovery.

FP8 KV cache is separate from weight quantization. Compresses KV cache values from FP16 to FP8, halving memory per cached token. Does not touch model weights. Requires Hopper (H100) or later. AWQ/GPTQ and FP8 KV are orthogonal and can be combined. Both dequantize at compute time.
...
INT8-AWQ improves decode throughput by 70% over FP16 (conc=1: 62.8 vs 37.0 tok/s) but prefill throughput by only 26% (31.1 vs 24.6 tok/s). Decode is memory-bandwidth-bound, so smaller weight tensors yield faster HBM reads per token. Prefill is compute-dominated under these workload parameters. INT8 matmul throughput on T4 does not scale proportionally, consistent with limited INT8 Tensor Core utilization on this hardware (inferred from throughput asymmetry, not directly profiled).
...
INT8-AWQ: +0.17% perplexity, zero accuracy drop, zero qualitative failures. Indistinguishable from FP16 across all 10 test prompts.

INT4-GPTQ: +7.8% perplexity, -1.31% accuracy drop, 5/10 qualitative failures. Not subtle:
...
INT4 holds up on constrained token distributions (reasoning, high conditional probability per step) but breaks on flatter distributions (creative writing, open-ended generation). The model confidently generates wrong tokens, not uncertain ones. Perplexity catches the aggregate distribution shift. Task accuracy only catches shifts that cross a decision boundary. You need both.
...
INT8-AWQ reduces model size by 2.56 GiB (44%), freeing that for KV cache. At 4K sequence length, max concurrent requests goes from 47 to 66 (40% capacity gain). For throughput-maximizing deployments, this capacity gain matters more than per-request latency reduction.

---

## 5. Cost Model ($/Million Tokens)

Instance: g4dn.xlarge, on-demand $0.526/hr. Using decode-heavy throughput at conc=8 as reference operating point.
...
INT8-AWQ: $0.552 to $0.411/M tokens (25.4% reduction), +0.17% perplexity. INT4-GPTQ: $0.304/M tokens (44.9% reduction), +7.8% perplexity with qualitative degradation on creative writing, code, and factual Q&A.
...
Speedup grows with system prompt length up to 1000T (18x), then drops at 2000T (12.4x). At 2000T, uncached tokens still pay O(sequence_length) attention against all cached KV entries, and cross-attention cost starts to dominate.

At 0% hit rate, prefix caching has zero overhead. TTFT was indistinguishable from cache-off (2192ms vs 2193ms at syslen=1000). Validates V1's decision to default it on.

Benefit becomes significant (>20% TTFT reduction) at 25% hit ratio (1758ms vs 2193ms at syslen=1000).

When it matters: chatbots with shared system prompts, RAG with common document prefixes, agent frameworks with standardized scaffolding. When it doesn't: per-user custom prompts with low reuse.

---

## 7. Speculative Decoding

From Day 14 experiments. Tested n-gram (prompt pattern matching) and draft model (Qwen2.5-0.5B-Instruct) paths.

Compatibility: TinyLlama (vocab 32,000) failed against Qwen (vocab 151,936). vLLM rejected at startup. Qwen2.5-0.5B shares the same tokenizer and launched successfully.
...
Every configuration was net negative. N-gram was 7-8% slower than baseline across all tasks. Draft model was worse: throughput collapsed to 15.1 tok/s (less than half of baseline 37.5) despite 39.3% acceptance. The 3B target is only 6x larger than the 0.5B draft, so draft forward passes are a significant fraction of total cost. With a 70B target and 1B draft (70x ratio), draft overhead would be negligible.

Speedup degrades with concurrency (0.92x at conc=1 to 0.86x at conc=8). Continuous batching already raises GPU utilization during decode, closing the gap spec decode targets.

Spec decode is a low-concurrency, memory-bound-regime optimization for larger models on more powerful hardware. On T4 with a 3B model, individual decode steps are already fast (~27ms ITL), leaving no room for improvement.

---

## 8. Recommendations

Every recommendation names its target SLO. Four axes: latency (p99 TTFT/ITL), throughput (tok/s), cost ($/M tokens), quality floor (perplexity ceiling, task accuracy minimum).
...
T4-class cost-constrained fleet (my hardware): INT8-AWQ as default. 40% more concurrent capacity (S4) at 25.4% lower $/M tokens (S5) with 0.17% perplexity increase (S3). Prefix caching on by default for repeated system prompts. Spec decode off.

H100-class latency-sensitive fleet (extrapolated, not measured): FP16 or BF16 baseline. Higher HBM bandwidth (3.35 TB/s vs 320 GB/s) reduces urgency of weight quantization for decode throughput, but INT8-AWQ still helps for capacity and cost. FP8 KV cache becomes the primary new capacity lever (S9, theoretical). Spec decode viable at low concurrency with appropriate draft model sizing (target-to-draft ratio needs to be large).
...
Do not deploy INT4-GPTQ for quality-critical workloads. +7.8% perplexity and 5/10 qualitative failure rate are unacceptable for code generation, medical summarization, or multi-step reasoning where creative/fluency outputs matter. Cost savings do not justify shipping degraded outputs.
...
INT8-AWQ gives 40% more concurrent capacity than FP16 (S4) at 25.4% lower $/M tokens (S5) with +0.17% perplexity (S3). Prefix caching cuts TTFT on system-prompt-heavy traffic by up to 18x (S6). Spec decode stays off, net negative at all concurrency levels and task types on this hardware (S7).
